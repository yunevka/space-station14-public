ent-BasePowerCell = { ent-BaseItem }
    .desc = { ent-BaseItem.desc }
    .suffix = { "" }
ent-PowerCellPotato = картофельная батарея
    .desc = Кто-то воткнул два гвоздя и несколько проводов в большую картофелину. Каким-то образом это обеспечивает небольшой заряд. Вы можете запихнуть его в слот среднего размера.
    .suffix = { "" }
ent-PowerCellSmall = батарея малой ёмкости
    .desc = Перезаряжаемый элемент питания. Самый дешёвый из всех доступных.
    .suffix = Полный
ent-PowerCellSmallPrinted = { ent-PowerCellSmall }
    .suffix = Пустой
    .desc = { ent-PowerCellSmall.desc }
ent-PowerCellMedium = батарея средней ёмкости
    .desc = Перезаряжаемый элемент питания. Популярный и надёжный вариант.
    .suffix = Полный
ent-PowerCellMediumPrinted = { ent-PowerCellMedium }
    .suffix = Пустой
    .desc = { ent-PowerCellMedium.desc }
ent-PowerCellHigh = батарея высокой ёмкости
    .desc = Стандартизированный перезаряжаемый элемент питания. Этот премиальный накопитель большой емкости вмещает на 50% больше энергии, по сравнению с конкурентами.
    .suffix = Полный
ent-PowerCellHighPrinted = { ent-PowerCellHigh }
    .suffix = Пустой
    .desc = { ent-PowerCellHigh.desc }
ent-PowerCellHyper = батарея гипер ёмкости
    .desc = Стандартизированный перезаряжаемый элемент питания. Выглядит как редкий и мощный прототип.
    .suffix = Полный
ent-PowerCellHyperPrinted = { ent-PowerCellHyper }
    .suffix = Пустой
    .desc = { ent-PowerCellHyper.desc }
ent-PowerCellMicroreactor = маленькая микрореакторная батарея
    .desc = Стандартизированный перезаряжаемый микрореактор. Предназначен для устройств с низким энергопотреблением, он медленно заряжается сам по себе.
    .suffix = Полный
ent-PowerCellAntiqueProto = прототип древней батареи
    .desc = Маленький самозаряжающийся элемент питания. Использовался в старых разработках лазерного оружия.
    .suffix = { "" }
ent-PowerCellMicroreactorPrinted = микрореакторная батарея
    .desc = Перезаряжаемая стандартизированная микрореакторная ячейка. Имеет меньшую емкость, но медленно заряжается самостоятельно.
ent-PowerCellCompactDefib = батарея компактного дефибрилятора
    .desc = Небольшой аккумулятор, который самостоятельно перезаряжается. Используется в старых исследованиях лазерного оружия.
ent-PowerCageHigh = большая силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Увеличенная емкость для увеличения уровня мощности.
ent-PowerCageHighEmpty = большая силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Увеличенная емкость для увеличения уровня мощности.
    .suffix = Пустая
ent-PowerCageMedium = средняя силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Золотой стандарт мощности и стоимости.
ent-PowerCageMediumEmpty = средняя силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Золотой стандарт мощности и стоимости.
    .suffix = Пустая
ent-PowerCageSmall = маленькая силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Это самый дешевый вид, который вы можете найти.
ent-PowerCageSmallEmpty = маленькая силовая ячейка
    .desc = Перезаряжаемый блок питания для больших устройств. Это самый дешевый вид, который вы можете найти.
    .suffix = Пустая

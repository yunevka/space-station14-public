...
namespace Content.Server.Imperial.Telebaton
{
  public sealed class TelebatonSystem : EntitySystem 
  {
    ...
    private void RetractTelebaton(EntityUid uid, TelebatonComponent component) 
    {
      ...
      if (TryComp<AppearanceComponent>(component.Owner, out var appearance) && TryComp<ItemComponent>(component.Owner, out var item)) 
      {
        _item.SetHeldPrefix(component.Owner, "retracted", item);
        _appearance.SetData(uid, ToggleVisuals.Toggled, false, appearance);
      }

      _audio.PlayPvs(component.stunSound);
    ...
    private void ExtendTelebaton(EntityUid uid, TelebatonComponent component, EntityUid user) 
    {
      ...
      if (EntityManager.TryGetComponent<AppearanceComponent>(component.Owner, out var appearance) && EntityManager.TryGetComponent<ItemComponent>(component.Owner, out var item)) 
      {
        _item.SetHeldPrefix(component.Owner, "extended", item);
        _appearance.SetData(uid, ToggleVisuals.Toggled, true, appearance);
      }

        _audio.PlayPvs(component.extendSound);
